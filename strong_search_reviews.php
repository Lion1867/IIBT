...
if (isset($_POST['searchData'])) {
    // Получаем значения всех пяти полей из поискового запроса
    $searchData = $_POST['searchData'];

    // SQL-запрос для поиска в отзывах по всем пяти полям
    $sql = "SELECT * FROM reviews_moder WHERE 
            user_surname LIKE '%$searchData[0]%' AND
            user_name LIKE '%$searchData[1]%' AND
            user_patronymic LIKE '%$searchData[2]%' AND
            company_position LIKE '%$searchData[3]%' AND
            organization LIKE '%$searchData[4]%'";
